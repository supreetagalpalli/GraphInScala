...
object CircularBufferDetection {
  ...
  def containsCycle[V](startVertex: V, graph: Graph[V]): Boolean = {
    val tortoise = LazyList.iterate(List(startVertex))(vList => moveOnce(vList,graph)).map(_.headOption)
    val hare = LazyList.iterate(List(startVertex))(vList => moveOnce(moveOnce(vList,graph),graph)).map(_.headOption)
